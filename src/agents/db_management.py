...
from src.database.chroma_client import (
    get_all_uploaded_files as get_files_standalone, 
    delete_document_by_name,
    get_file_content_by_name  # Hàm mới import vào đây
)
from concurrent.futures import ThreadPoolExecutor 

import time  # <-- THÊM DÒNG NÀY ĐẦU FILE
...
def db_management_node(state):
    ...
    # 1. Lấy danh sách file đang có trong database
    ...
    if action == "LIST":
        ...
        if filename_to_summary in files:
            ...
            if not file_content.strip():
                ...
                # CHUYỂN ĐỔI SANG ĐA LUỒNG TẠI ĐÂY:
                # Chuẩn bị dữ liệu đầu vào cho các luồng
                worker_args = [(llm, raw_summary_prompt, filename_to_summary, chunk, idx) for idx, chunk in enumerate(text_chunks)]
                
                # Kích hoạt ThreadPoolExecutor để gửi tất cả các chunk lên Groq cùng 1 lúc
                # SỬA THAM SỐ MAX_WORKERS THÀNH 1 (Chạy tuần tự an toàn tuyệt đối)
                with ThreadPoolExecutor(max_workers=1) as executor:
                    partial_summaries = list(executor.map(invoke_chunk_summary, worker_args))

                
                # Lọc bỏ các phần bị lỗi (nếu có)
                partial_summaries = [s for s in partial_summaries if s.strip()]
                
                # Bước Reduce: Gom tổng hợp thành bản tổng thể mạch lạc
        ...
        if filename_to_delete in files:
            ...
            client.heartbeat() 
            ai_msg = f"Đã thực hiện xóa thành công toàn bộ dữ liệu của file '{filename_to_delete}' khỏi hệ thống và giải phóng bộ nhớ cache."
        else:
